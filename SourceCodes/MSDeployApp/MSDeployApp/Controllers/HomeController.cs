...
namespace MSDeployApp.Controllers
{
    ...
    {
        public string bingaccesskey, textaccesskey;
        ...
        public ActionResult SqlDBFn(string azuid, string azpwd, string dbsrc, string dbuid, string dbpwd, string pbid, string pbpwd)
        {
            ...
            using (PowerShell PowerShellInstance1 = PowerShell.Create())
            {
                ...
                var bingkeypath = Server.MapPath(@"~\App_Data\BingKey.txt");
                var textkeypath = Server.MapPath(@"~\App_Data\TextKey.txt");
                ...
                {
                    bingaccesskey = result1.Members["Key1"].Value.ToString();
                }
                PowerShellInstance1.Commands.Clear();
                ...
                {
                    textaccesskey = result2.Members["Key1"].Value.ToString();
                }
                PowerShellInstance1.Commands.Clear();

                var keyfilepath = Server.MapPath(@"~\App_Data\KeysFile.txt");
                var sw = new StreamWriter(keyfilepath);
                sw.Write(bingaccesskey + ',' + textaccesskey);
                sw.Flush();
                sw.Close();
                ...
                var bingtext = System.IO.File.ReadAllText(keyfilepath);
                var strarray = bingtext.Split(',');
                if (strarray.Length > 1)
                {
                    stracskey1 = strarray[0];
                    stracskey2 = strarray[1];
                }


                //ARM Template JSON Logic App Dynamic Values
                string jsonFile7 = Server.MapPath(@"~\App_Data\ARMTemplate.json");
                string json7 = System.IO.File.ReadAllText(jsonFile7);
                var jObject7 = JObject.Parse(json7);
                jObject7["parameters"]["sqlServer"]["defaultValue"] = dbsrc;
                jObject7["parameters"]["sqlUser"]["defaultValue"] = dbuid;
                jObject7["parameters"]["sqlPassword"]["defaultValue"] = dbpwd;
                jObject7["parameters"]["sqlDatabase"]["defaultValue"] = dbname;
                jObject7["parameters"]["storage_accessKey1"]["defaultValue"] = stracskey1;
                jObject7["parameters"]["storage_accessKey2"]["defaultValue"] = stracskey2;
                string output7 = JsonConvert.SerializeObject(jObject7, Formatting.Indented);
                System.IO.File.WriteAllText(jsonFile7, output7);


                string TempPath7 = Server.MapPath(@"~\App_Data\ARMTemplate.json");
                PowerShellInstance1.AddScript("$resourceGroupName = '" + rgname + "'");
                //var logpath = Server.MapPath(@"~\App_Data\log.txt");
                PowerShellInstance1.AddScript("New-AzureRmResourceGroupDeployment -ResourceGroupName $resourceGroupName -TemplateFile " + TempPath7);
                ...
                string json0 = System.IO.File.ReadAllText(jsonFile0);
                ...
                System.IO.File.WriteAllText(jsonFile0, output0);

                //Deploying ServerDB Template 
                string TempPath0 = Server.MapPath(@"~\App_Data\ServerDB.json");
                PowerShellInstance1.AddScript("$resourceGroupName = '" + rgname + "'");
                PowerShellInstance1.AddScript("New-AzureRmResourceGroupDeployment -ResourceGroupName $resourceGroupName -TemplateFile " + TempPath0);
                ...
                string jsonFile1 = Server.MapPath(@"~\App_Data\ML_Resource.json");
                string json1 = System.IO.File.ReadAllText(jsonFile1);
                ...
                string output2 = JsonConvert.SerializeObject(jObject1, Formatting.Indented);
                System.IO.File.WriteAllText(jsonFile1, output2);


                //Deploying ML_Resource LogicApp
                string TempPath4 = Server.MapPath(@"~\App_Data\ML_Resource.json");
                PowerShellInstance1.AddScript("$resourceGroupName = '" + rgname + "'");
